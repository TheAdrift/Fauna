...
--a dictionary containing control aliases.
push =
{
	up = "w",
	left = "a",
...
FloorCollider = Fill:extend
{
	...
	onUpdate = function(self, dT)
		...
		if the.keys:justPressed (push.dodge) and not state.isDodge and not state.inAir then
			state.isDodge = true
			state.queue = .5
			the.app.view.pBody.jumpSpeed = default.jumpForce/3
			state.inAir = true
	...
	onCollide = function( self, other ) 
		...
		if other.traverse == "wall" then 
			other.displace( other, self ) 
		elseif other.traverse == "jumpable" then 
			if state.inAir ~= true then 
				other.displace( other, self ) 
			end
		elseif other.traverse == "pit" then 
			if state.inAir ~= true then 
				if math.abs(self.x - other.x) < 16 and math.abs(self.y - other.y) < 16 then 
					print('X different' .. math.abs(self.x - other.x))
					print('Y different' .. math.abs(self.y - other.y)) 
					self:die()
				end
			end
		end
		
		if other.goal == true then 
		
			the.app.view.stageWon = true
			if the.app.view.stageWon then 
			   print('you should be winning!') 
			else 
			   print('it broke! :c') 
			end
		end
...
BodyCollider = Fill:extend
{
	...
	onUpdate = function (self, dT)
		...
		self.x = the.app.view.pFloor.x
		self.y = the.app.view.pFloor.y - (self.height - (the.app.view.pFloor.height/2)) - self.altitude
		...
		if not the.app.view.pFloor.active then 
			self:die()
		end
	end
}